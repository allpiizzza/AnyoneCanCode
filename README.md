...
- **step1** — ① Replit 열고 파일 세 개 만들기(화면 세 덩어리 `.flow` + 이름 정확히 경고) → ② 프롬프트 붙여넣기(`.more`: 이 프롬프트가 왜 이렇게 생겼는지) → ③ 받은 코드를 Replit에 옮기기(복사 버튼 → 전체 선택 → 붙여넣기 → Run) → ④ 화면 확인 4가지 → 고치기 → 질문 5개 → 🎤 45분 운영
