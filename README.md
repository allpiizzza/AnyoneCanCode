...
`index.html` · `checklist.html`을 뺀 나머지 6개 문서는 **워크샵 당일 0시에 자동으로 열립니다.** 그 전에는 잠금 화면(`.lockbox`)이 본문 자리를 차지하고, **참가자 코드**를 넣으면 바로 열립니다.
...
**templates.html** — 역할·상황/맥락·요청·형식 4단계 빈칸 템플릿 + 각 칸의 의미 표 + **"한 번에 다 시키지 말고, 쪼개서 시키세요"** + 채워 넣은 예시.

쪼개기 섹션은 ① 한꺼번에 시키면 생기는 일 4가지(조건 누락 · 긴 출력에서 앞뒤 어긋남 · 원인 특정 불가 · 되돌릴 지점 없음) ② 작업 리듬 4단계(한 가지만 요청 → 화면 확인 → "여기까지 좋아" → 망가지면 되돌리기) ③ 이어서 요청할 때 쓰는 `next-step.txt` 프롬프트 로 구성됩니다.

이어지는 **"그럼 맨 처음엔? — 인터뷰로 계획부터 세우세요"** 섹션이 그 예외를 다룹니다. 핵심은 **"나중에 따로 만들면 서로 안 맞는 것들만 한 번에 묶는다"** 한 줄이고, 묶을 것 4가지(파일 이름 · 페이지 이동 · 데이터 항목과 순서 · 전체 톤)를 표로 보여줍니다. 그다음 **쪼개는 대상은 코드를 바꾸는 요청이지 대화가 아니라는 구분**(계획은 말로 여러 번 고치고, 코드 생성은 한 번), 3단계 흐름, 프롬프트 두 개(`interview.txt` — AI가 먼저 질문하게 하기 / `build-from-plan.txt` — 계획 중 이번 범위만 잘라 요청), 그리고 "한 번에도 크기 한계가 있다(화면 한 세트까지 = STEP 1과 STEP 2의 경계)"는 경고가 붙습니다.

> 마지막 tip 콜아웃은 **인터뷰 단계를 워크샵 당일에는 건너뛰어도 된다**고 안내합니다. 당일에는 계획이 이미 정해져 있고 시간이 빠듯하기 때문입니다. 이 섹션은 "끝나고 내 것을 만들 때" 쓰는 자산으로 두세요.

> 이 섹션에는 **"AI가 집중력을 잃는다"는 표현을 바로잡는 콜아웃**이 들어 있습니다. 실제로는 주의가 흐트러지는 게 아니라 지켜야 할 조건이 늘수록 일부를 놓칠 확률이 올라가고 출력이 길수록 어긋날 자리가 늘어나는 것이라고 설명합니다. 문구를 고칠 때 이 구분을 뭉개지 마세요.
