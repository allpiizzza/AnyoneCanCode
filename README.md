...
쪼개기 섹션은 ① 한꺼번에 시키면 생기는 일 4가지(조건 누락 · 긴 출력에서 앞뒤 어긋남 · 원인 특정 불가 · 되돌릴 지점 없음) ② 작업 리듬 4단계(한 가지만 요청 → 화면 확인 → "여기까지 좋아" → 망가지면 되돌리기) ③ 이어서 요청할 때 쓰는 `next-step.txt` 프롬프트 ④ "처음 뼈대는 오히려 한 번에" 예외 로 구성됩니다.
