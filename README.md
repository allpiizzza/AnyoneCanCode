...
**checklist.html** — 클릭 시 취소선이 그어지는 체크박스 4그룹(총 22개): ① 진행자 사전 준비 7 ② 참가자 사전 공지 6 ③ 당일 계정 세팅 순서 4 ④ 당일 준비물 5. 이어서 **당일 진행 대본**(구간별 할 일 + "이 말은 꼭" 한 줄), **🎤 시간이 밀릴 때 자르는 순서**(여유과제 → STEP 1 커스터마이징 → STEP 2 개별해결 순으로 자르고 **STEP 3 배포는 절대 자르지 않음**), 체크포인트 타이밍 콜아웃(계정생성→화면완료→Sheets연동→배포 4회 확인).
