...
| `.callout.host` | 🎤 진행자용 안내 (청록). 참가자용 `.tip`과 구분됩니다 |
...
```
badge · h1 · subtitle · lead
.brief          이 문서에서 할 일 (목표 / 시간 / 준비물 / 끝나면)
"지금 무슨 일이 일어나는 건가요"  개념 설명 + .flow 다이어그램
본론            프롬프트 · 단계 가이드
.callout.host   🎤 진행자 운영 요령 (시간 배분, 멈출 지점, 자를 순서)
.qa             자주 나오는 질문
.pagenav
```

- **참가자용과 진행자용을 색으로 구분합니다.** `.callout.tip`(초록)은 참가자에게 하는 조언, `.callout.host`(청록, 🎤)는 진행자용 운영 지침입니다. 진행자용도 숨기지 않고 그대로 노출합니다 — 참가자가 읽어도 문제될 내용이 없고, 오히려 "지금 왜 이렇게 진행하는지" 이해에 도움이 됩니다
- **`.qa`는 `<details>`라 JS 없이 접힙니다.** 문서가 길어져도 훑는 데 방해가 되지 않게 하는 장치입니다
...
각 STEP 페이지에 추가로 들어 있는 것:

- **step1** — Replit 화면 세 덩어리 `.flow` / "AI가 준 코드를 Replit에 옮기는 순서" 4단계(파일 먼저 만들기 → 코드 블록 복사 버튼 쓰기 → 전체 선택 후 붙여넣기 → Run) / 🎤 45분 운영(0-5-15-25분 배분, **대신 고쳐주지 말 것**) / 질문 6개
- **step2** — 🔑 "지금 무슨 일이 일어나는 건가요" 화면→웹앱→시트 `.flow` + Apps Script가 무엇인지 / 🎤 운영(시트는 다 함께, 배포는 시연 먼저, 30분 넘으면 넘기기) / 질문 6개(시트 공개 범위, "모든 사용자" 안전한가, 재배포 누락 등)
- **step3** — Replit→GitHub→Vercel `.flow` + "왜 두 단계나 거치나" / 고친 게 안 보일 때 확인 순서 ①Push ②GitHub 시각 ③Vercel 배포 / 질문 6개(주소 변경, 요금, Public 공개 범위, 저장소 안 보임, 흰 화면)
- **errors** — 상단에 3단계 `.flow`(콘솔 열기 → 빨간 글씨 복사 → AI에게) / **5. 진행자를 위한 대응 원칙**(대신 고쳐주지 않기, 3분 규칙, 같은 에러 2명이면 전체 공지, 먼저 끝낸 사람을 조교로, 막힌 채 넘어가도 됨) / 질문 4개
- **extra** — 🎤 속도 차가 가장 큰 구간이라 진도를 맞추지 말 것, "색·폰트 바꾸기"를 먼저 권할 것, 마지막 10분은 서로 주소 공유
